...
public class EmpleadoDao implements EmpleadoInterface{
		...
		public boolean alta(Empleado empleado) {
			try {
				String alta="Insert into tEmpleado values(null,?,?,?,?,?);";
				this.jdbcTemplate.update(alta,empleado.getDni(),empleado.getNombre(),empleado.getApellidos(),empleado.getTelefono(),empleado.getCorreo());
				return  true;
		...
		public boolean baja(int idEmpleado) {
			try {
				String baja="Delete from TEmpleado where idEmpleado=?;";
				this.jdbcTemplate.update(baja,idEmpleado);
				return true;
			}catch(DataAccessException ex) {ex.printStackTrace();}
			return false;
		}
		public boolean modificacion(Empleado empleado) {
			try {
				String modificacion="Update TEmpleado set dni=?,nombre=?,apellidos=?,telefono=?, correo=? where idEmpleado=?";
				this.jdbcTemplate.update(modificacion,empleado.getDni(),empleado.getNombre(),empleado.getApellidos(),empleado.getTelefono(),empleado.getCorreo(),empleado.getIdEmpleado());
				return true;
			}catch(DataAccessException ex) {ex.printStackTrace();}
			return false;
		}
		public Empleado consulta(int idEmpleado) {
			try {
				...
				Empleado empleado=this.jdbcTemplate.queryForObject(consulta, new EmpleadoMapper(), idEmpleado);
				String consultaPrestamos="Select * from tPrestamo where idEmpleado=?";
				ArrayList<Prestamo>prestamos=(ArrayList)this.jdbcTemplate.query(consultaPrestamos,new PrestamoMapper(), idEmpleado);
		...
		public ArrayList<Empleado> consulta() {
			try {
				String consulta="Select * from tEmpleado";
				ArrayList<Empleado>empleados=(ArrayList)this.jdbcTemplate.query(consulta, new EmpleadoMapper());
				for(Empleado e:empleados) {
					String consultaPrestamos="Select * from tPrestamo where idEmpleado=?";
					ArrayList<Prestamo>prestamos=(ArrayList)this.jdbcTemplate.query(consultaPrestamos,new PrestamoMapper(), e.getIdEmpleado());
					e.setPrestamos(prestamos);
